...
def crear_pedido(conexion):
    ...
    #Peidmos que introduzca los datos del pedido
    id_pedido = int(input('Ingresa el ID del nuevo peiddo: '))
    id_cliente = input('Ingresa el ID cliente (5 caracteres): ')
    fecha_pedido = input('Ingresa la fecha del pedido (YYYY-MM-DD HH:MM:SS): ')
    fecha_entrega = input('Ingresa la fecha de entrega (YYYY-MM-DD HH:MM:SS): ')

    nuevo_pedido = f"""
        INSERT INTO pedido (idpedido, idcliente, fechapedido, fechaentrega)
        VALUES (%s, %s, %s, %s;"""
    
    valores_pedido = (id_pedido, id_cliente, fecha_pedido, fecha_entrega)
    cursor.execute(nuevo_pedido, valores_pedido)
    ...
    while True:
        id_producto = int(input('Ingresa el ID del producto para el detalle del pedido (o 0 para terminar): '))
        if id_producto == 0:
            break

        precio = float(input('Ingresa el precio del producto: '))
        unidades = int(input('Ingresa las unidades del producto: '))
        descuento = float(input('Ingresa el descuento: '))

        valores_detalle = (id_pedido,id_producto, precio, unidades, descuento)
        cursor.execute(nuevo_detalle, valores_detalle)

    conexion.commit()
    print('El pedido y sus detalles han sido creados con exito\n')
    cursor.close()
...
def leer_pedido(conexion):
    ...
    mostrar_pedidos = """
        SELECT p.idpedido, p.idcliente, p.fechapedido, p.fechaentrega, d.idproducto, d.precio, d.unidades, d.descuento
        FROM pedido p
        LEFT JOIN detalle d ON p.idpedido = d.idpedido;"""
    
    cursor.execute(mostrar_pedidos)
    recuperar_registros_pedido = cursor.fetchall()

    for i  in recuperar_registros_pedido:
        print(f"Pedido ID: {recuperar_registros_pedido[0]}, Cliente ID: {recuperar_registros_pedido[1]}, Fecha Pedido: {recuperar_registros_pedido[2]}, Fecha Entrega: {recuperar_registros_pedido[3]},
            Producto ID: {recuperar_registros_pedido[4]}, Precio: {recuperar_registros_pedido[5]}, Unidades: {recuperar_registros_pedido[6]}, Descuento: {recuperar_registros_pedido[7]}")
...
def actualizar_pedido(conexion):
    ...
    id_pedido = input('Ingresa el ID del pedido que deseas actualizar: ')

    while True:
        
  # Preguntar al cliente que quiere editar segun el ID pedido
        print("\n¿Qué desea actualizar?")
        print("1. Fecha de entrega del pedido")
        print("2. Detalles del producto en el pedido")
        print("0. Salir")
        opcion = input("Ingrese una opción (1, 2, o 0 para salir): ")

        if opcion == '1':
            # Actualizar solo la fecha de entrega
            nueva_fecha_entrega = input('Ingrese la nueva fecha de entrega (YYYY-MM-DD HH:MM:SS): ')
            actualizar_pedido = f"""
                UPDATE pedido
                SET fechaentrega = %s
                WHERE idpedido = %s;
            """
            cursor.execute(actualizar_pedido, (nueva_fecha_entrega, id_pedido))
            print('Fecha de entrega actualizada con éxito.')

        elif opcion == '2':
            # Actualizar detalles del producto
            while True:
                id_producto = int(input('Ingrese el ID del producto en el detalle a actualizar (o 0 para terminar): '))
                if id_producto == 0:
                    break
                nuevo_precio = float(input('Ingrese el nuevo precio: '))
                nuevas_unidades = int(input('Ingrese las nuevas unidades: '))
                nuevo_descuento = float(input('Ingrese el nuevo descuento: '))

                actualizar_detalle_sql = f"""
                    UPDATE detalle
                    SET precio = %s, unidades = %s, descuento = %s
                    WHERE idpedido = %s AND idproducto = %s;
                """
                cursor.execute(actualizar_detalle_sql, (nuevo_precio, nuevas_unidades, nuevo_descuento, id_pedido, id_producto))
                print('Detalle del producto actualizado con éxito.')

        elif opcion == '0':
            print("Saliendo del proceso de actualización.")
            break
        else:
            print("Opción no válida. Intente nuevamente.")

    conexion.commit()
    print('Pedido y detalles actualizados con éxito\n')
    cursor.close()
...
def eliminar_pedido(conexion):
    ...
    eliminar_detalle = """
        ...
        WHERE idpedido = %s;
    """
    cursor.execute(eliminar_detalle, (id_pedido,))
    ...
    eliminar_pedido = """
        ...
        WHERE idpedido = %s;
    """
    cursor.execute(eliminar_pedido, (id_pedido,))
